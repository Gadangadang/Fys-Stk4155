...
def SGD_optimization_test(X, y):
    ...
    X_train, X_test, y_train, y_test = mean_scale_new(
    ...
    SGD_regression = SGD(
        X_train, y_train, eta_vals[0], m=0, num_epochs=num_epochs)
    ...
    ax = sns.heatmap(test_MSE, xticklabels=bsp_vals, yticklabels=np.log10(
        eta_vals), annot=True, ax=ax, cmap="viridis")
...
def SGD_test_learning_rate(X, y):
    ...
    SGD_regression = SGD(
        X_train, y_train, eta_vals[0], m=m, num_epochs=num_epochs)
    ...
    print("test min arg:", eta_vals[np.argmin(
        SGD_MSE_test)], "+-", eta_vals[1] - eta_vals[0])
...
def SGD_convergence_rate(X, y):
    ...
    for i, bsp_pct in enumerate(bsp_vals):
        ...
        plt.plot(epoch_arr, MSE_arr[i], "-o", markersize=4,
                 label=f"m = {SGD_regression.m} ({bsp_pct*100:g}%)")
    ...
    plt.legend(fontsize=13)
    plt.tight_layout(pad=1.1, w_pad=0.7, h_pad=0.2)
    plt.savefig("../article/figures/SGD_batch_size_convergence.pdf",
                bbox_inches="tight")
    ...
    ax = plt.gca()
    plt.hlines(MSE_OLS, 1, num_epochs, linestyle="--",
               color="black", label="OLS MSE")
...
def SGD_VS_OLS():
    ...
    X_F_train, X_F_test, y_train, y_test = train_test_split(
        X_full, z, test_size=split)
    X_F_train, X_F_test, y_train, y_test = mean_scale_new(
        X_F_train, X_F_test, y_train, y_test)
    ...
    for i, n in enumerate(n_arr):
        ...
        # SGD
        np.random.seed(4155)  # RN Seed
        SGD_regression = SGD(X_train, y_train, eta_val,
...
def SGD_momentum_optimization(X, y):
    ...
    fig, ax = plt.subplots(figsize=(7, 7))
    # df = pd.DataFram(test_MSE), columns
    ax = sns.heatmap(test_MSE, xticklabels=gamma_vals,
                     yticklabels=eta_vals, annot=True, ax=ax, cmap="viridis")
    # ax.xaxis.set_major_formatter(FormatStrFormatter('%.2f'))
