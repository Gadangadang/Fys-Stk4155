    ...
    * Prediction
    ...
    def __init__(
        ...
        if activation == "relu" or activation == "leaky_relu":
            self.scaled_weight = [
                num_hidden_nodes ** 2,
                num_hidden_nodes * self.num_features,
    ...
    def create_layers(self):
        ...
        """
        self.layers_a = [
            np.zeros((self.N, self.num_hidden_nodes), dtype=np.float64)
            for i in range(self.num_hidden_layers)
        ]  # Intialized with the hidden layers

        self.layers_a.insert(0, self.X.copy())  # Add input layer
        self.layers_a.append(
            np.zeros((np.shape(self.t)), dtype=np.float64)
        )  # Add output layer
    ...
    def create_biases_and_weights(self):
        ...
        bias_shift = 0.1
        self.weights = [
            np.random.randn(num_hidden_nodes, num_hidden_nodes)
            / (self.scaled_weight[0])
            for i in range(num_hidden_layers - 1)
        ]
        self.weights.insert(
            0, np.random.randn(num_hidden_nodes, num_features) / (self.scaled_weight[1])
        )

        # insert unused weight to get nice indexes
        self.weights.insert(0, np.nan)
        self.weights.append(
            np.random.randn(self.num_output_nodes, num_hidden_nodes)
            / (self.scaled_weight[2])
        )

        self.bias = [
            np.ones(num_hidden_nodes) * bias_shift for i in range(num_hidden_layers)
        ]
        ...
        if self.check_grad > self.tol and np.isfinite(self.check_grad):
            for l in range(1, self.L):
                self.vel_weights[l] = self.gamma * self.vel_weights[l] + self.eta * (
                    self.local_gradient[l].T @ self.layers_a[l - 1]
                    + self.weights[l] * self.lmbd
                )
                self.weights[l] -= self.vel_weights[l]

                self.vel_bias[l] = (
                    self.gamma * self.vel_bias[l]
                    + self.eta * np.mean(self.local_gradient[l], axis=0)
                    + self.lmbd * self.bias[l]
                )
    ...
    def backpropagation(self):
        self.local_gradient[-1] = self.cost_der(
            self.layers_a[-1]
        ) * self.last_activation_der(self.layers_z[-1])
        self.check_grad = np.linalg.norm(self.local_gradient[-1] * self.eta)
        for l in reversed(range(1, self.L - 1)):
            self.local_gradient[l] = (
                self.local_gradient[l + 1]
                @ self.weights[l + 1]
                * self.activation_der(self.layers_z[l])
            )
    ...
    def train_network_stochastic(self, epochs, plot=False):
        ...
        while (
            epoch < epochs
            and self.check_grad > self.tol
            and np.isfinite(self.check_grad)
        ):
            self.eta_func(epoch)
    ...
    def plot_score_history(self, name=None, legend=[]):
        ...
        plt.plot(
            np.linspace(0, self.num_epochs, self.num_epochs),
            self.score[: self.num_epochs],
            linestyle=linestyle,
            marker=marker,
        ...
        if self.score.shape[1] > 1:
            if len(legend) == 0:  # no legend in arg
                plt.legend(
                    ["category " + str(i) for i in range(self.score.shape[1])],
                    fontsize=13,
                )
        ...
        if isinstance(name, str):
            plt.savefig(
                f"../article/figures/{name}_score_history.pdf", bbox_inches="tight"
            )
        plt.show()
    ...
    def R2_score(self, X, target):
        ymod = self.predict(X)
        target = target
        return 1 - np.sum((target - ymod) ** 2) / np.sum(
            (target - np.mean(target, axis=0) ** 2)
        )
    ...
    def __str__(self):
        ...
        text += "Hidden nodes in network:       {} \n".format(
            self.num_hidden_nodes * self.num_hidden_layers
        )
